...
// Refined prop types: remove versions, only use proposal
interface NounsProposalDetailViewProps {
...
export const NounsProposalDetailView: React.FC<
  NounsProposalDetailViewProps
> = ({ proposal, goBack, currentBlock, loading, headingsFont, bodyFont }) => {
  ...
  let lastUpdated;
  if (proposal.lastUpdatedTimestamp) {
    lastUpdated = moment(
      Number(proposal.lastUpdatedTimestamp) * 1000
    ).fromNow();
  ...
  const isFirstVersion =
    proposal.createdTimestamp === proposal.lastUpdatedTimestamp;
  const lastUpdatedText = isFirstVersion
    ? `Created ${lastUpdated}`
    : `Updated ${lastUpdated}`;
        ...
        >
          <div className="flex flex-col gap-3">
            <div className="flex flex-col gap-2">
              ...
              <p
                className="font-medium text-base/[1.25]"
          ...
          <div className="flex flex-col gap-2 pb-8 font-normal">
            <h2
              className="font-medium text-sm/[1.25]"
              style={{ fontFamily: headingsFont }}
            >
              Description
            </h2>
