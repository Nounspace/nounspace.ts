...
const createMiniAppBootstrapSrcDoc = (targetUrl: string, bridgeId: string) => {
  ...
  const scriptLines = [
    ...
    "      if (TARGET_URL) {",
    "        try {",
    "          // Create or select a child iframe to load the mini-app",
    "          // This preserves the bootstrap realm with the injected provider",
    "          var childIframe = document.getElementById('miniapp-content-iframe');",
    "          if (!childIframe) {",
    "            childIframe = document.createElement('iframe');",
    "            childIframe.id = 'miniapp-content-iframe';",
    ...
    "            childIframe.sandbox = 'allow-scripts allow-same-origin allow-forms allow-popups allow-popups-to-escape-sandbox';",
    "            document.body.appendChild(childIframe);",
    "          }",
    "          childIframe.src = TARGET_URL;",
    "        } catch (err) {",
    "          console.error('Failed to create child iframe for mini-app:', err);",
    "        }",
    "      }",
    "    }, 0);",
    "  } catch (err) {",
