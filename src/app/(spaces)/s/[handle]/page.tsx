...
const loadUserSpaceData = async (
  ...
  if (!handle) {
    console.error("No handle provided to loadUserSpaceData");
    return {
      spaceOwnerFid: null,
  ...
  try {
    ...
    console.error("Error in loadUserSpaceData:", error);
    return {
      spaceOwnerFid: null,
      spaceOwnerUsername: null,
      spaceId: null,
...
const ProfileSpacePage = async ({
  ...
  let decodedTabName = tabNameParam;
  if (decodedTabName) {
    try {
      decodedTabName = decodeURIComponent(decodedTabName);
    } catch (error) {
      console.error("Error decoding tabName:", error);
      decodedTabName = undefined;
    }
  }

  const { spaceOwnerFid, spaceOwnerUsername, spaceId, tabName } =
    await loadUserSpaceData(handle, decodedTabName);
