...
export default function Space({
  ...
  const isScreenMobile = width ? width < MOBILE_BREAKPOINT : false;
  const showMobileContainer = mobilePreview && !isScreenMobile;
  ...
  const layoutFidgetProps = useMemo(() => {
    return {
      ...
      inEditMode: !isScreenMobile && editMode, // No edit mode on actual mobile
      ...
      <div className="w-full transition-all duration-100 ease-out">
        {showMobileContainer ? (
          <div className="flex justify-center">
            <div className="w-[390px] h-[844px] relative">{mainContent}</div>
          </div>
        ) : (
          mainContent
        )}
