...
// Memoized IconButton component to prevent unnecessary re-renders
...
export const MobileTokenSpace = ({
  ...
  const handleTabChange = useCallback((label: Pages) => {
    setTab(label);
  }, []);
  ...
  return (
    <div className="h-full w-full flex flex-col">
      ...
      <div className="flex flex-shrink-1 items-end justify-around w-full py-2 border-t border-t-gray-200">
        <IconButton size={20} icon={BsBarChartFill} label="Price" currentTab={tab} onClick={handleTabChange} />
        <IconButton size={26} icon={MdOutlineSwapHoriz} label="Swaps" currentTab={tab} onClick={handleTabChange} />
        <IconButton size={20} icon={IoIosChatboxes} label="Chat" currentTab={tab} onClick={handleTabChange} />
        <IconButton size={20} icon={FaLink} label="Links" currentTab={tab} onClick={handleTabChange} />
        <IconButton size={20} icon={SiFarcaster} label="Feed" currentTab={tab} onClick={handleTabChange} />
      </div>
    </div>
  );
};
