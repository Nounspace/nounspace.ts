  ...
  ({
    ...
    return (
      ...
      >
        ...
        <div className="text-xl mb-1">{getTabIcon(tab)}</div>
...
const MobileNavbar: React.FC<MobileNavbarProps> = ({
  ...
  const getTabLabel = useCallback(
    (tab: TabItem, index: number): string => {
      ...
      if (fidgetInstanceDatums && fidgetInstanceDatums[tab.id]) {
        ...
        if (fidgetData.fidgetType) {
          ...
          if (fidgetModule) {
            ...
            return (
              fidgetModule.properties.mobileFidgetName ||
  ...
  const handleScroll = useCallback(() => {
    ...
    const rightGradientOpacity = Math.min(
      (maxScroll - scrollLeft) / transitionThreshold,
      1
    );
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent) => {
      ...
      if (e.key === "ArrowLeft" || e.key === "ArrowRight") {
        ...
        const selectedElement = tabsListRef.current?.querySelector(
          `[value="${tabs[nextIndex].id}"]`
        );
        if (selectedElement) {
