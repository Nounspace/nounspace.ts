...
function TabBar({
  ...
  return (
    <TooltipProvider>
      <div className="flex flex-col md:flex-row justify-start md:h-16 z-3 bg-white w-full"> 
        {isTokenPage && contractAddress && (
          <div className="flex flex-row justify-start h-16 overflow-y-scroll w-full z-2 bg-white"> 
            <TokenDataHeader />
          </div>
        )}
        <div className="relative flex flex-auto h-16 z-1 bg-white"> 
          {isMobile && (
            <div className="absolute right-2 top-1/2 transform -translate-y-1/2 z-4 bg-white flex gap-2">
        ...
        {inEditMode && !mobilePreview && !isMobile ? (
          <div className="mr-36 flex flex-row z-5">
            <NogsGateButton
              onClick={() => handleCreateTab(generateNewTabName())}
        ...
        {!inEditMode && !mobilePreview && !isMobile && isEditable && (
          <div className="absolute right-0 top-1/2 transform -translate-y-1/2 mr-4 z-4">
            <Button
              onClick={() => setEditMode(true)}
