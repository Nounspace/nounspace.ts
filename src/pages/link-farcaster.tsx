...
import NounspaceFarcasterAuthenticator, {
  NounspaceDeveloperManagedSignerData,
} from "@/authenticators/farcaster/signers/NounspaceManagedSignerAuthenticator";
...
export default function LinkFarcaster() {
  ...
  const [AuthComponent, setAuthComponent] =
    useState<AuthenticatorInitializer<NounspaceDeveloperManagedSignerData>>();
  ...
  useEffect(() => {
    saveData({
      requestingWallet: isArray(router.query.wallet)
        ? undefined
        : router.query.wallet,
    });
