...
import { Card, CardHeader } from "@/common/ui/atoms/card";
...
export default function Profile({ profile }) {
  ...
  useEffect(() => {
    ...
    const getData = async () => {
      ...
      const resp = await neynarClient.fetchBulkUsers([profile.fid], { viewerFid: userFid! as number });
  ...
  useHotkeys(
    ["tab", "shift+tab"],
    () => {
      setFeedType(feedType === FeedTypeEnum.casts ? FeedTypeEnum.likes : FeedTypeEnum.casts);
  ...
  const renderRow = (item: CastWithInteractions, idx: number) => (
    <li key={item?.hash} className="border-b border-gray-700/40 relative flex items-center space-x-4 max-w-full md:max-w-2xl">
      ...
      <Tabs value={feedType} className="pt-2 pb-2 w-full max-w-full">
        <TabsList className="grid w-full grid-cols-2">
          {Object.keys(FeedTypeEnum).map((key) => {
            return (
              <TabsTrigger key={key} value={FeedTypeEnum[key]} className="text-foreground/80 text-center" onClick={() => setFeedType(FeedTypeEnum[key])}>
                {FeedTypeEnum[key]}
                {feedType !== FeedTypeEnum[key] && (
                  <div className="ml-4 text-foreground/80 hidden md:block">
                    Switch with &nbsp;
                    <kbd className="px-1.5 py-1 text-xs border rounded-lg bg-foreground/80 text-background/80">Tab</kbd>
                  </div>
                )}
              </TabsTrigger>
            );
      ...
      <Card className="max-w-2xl mx-auto bg-transparent border-none shadow-none">
        <CardHeader className="flex space-y-0">
          <div className="flex space-x-4 grid grid-cols-2 lg:grid-cols-3">
            ...
            {userFid !== profile.fid && <FollowButton username={profile.username} />}
          </div>
  ...
  const [fidgetConfigs, setFidgetConfigs] = useState([
    { f: <></>, resizeHandles: availableHandles, x: 0, y: 3, w: 6, minW: 4, maxW: 8, h: 7, minH: 6, maxH: 12 },
    {
      f: <ProfileHeader />,
    ...
    { f: <FrameFidget url={"https://far.cards/api/trade/user/4888"} />, resizeHandles: availableHandles, x: 6, y: 0, w: 3, minW: 2, maxW: 4, h: 6, minH: 3, maxH: 12 },
    { f: <FrameFidget url={"https://altumbase.com/degen/4888/dIVWKaIQZR"} />, resizeHandles: availableHandles, x: 6, y: 6, w: 3, minW: 2, maxW: 4, h: 4, minH: 3, maxH: 12 },
    { f: <FrameFidget url={"https://yoink.alfafrens.com/inviteCode/rollingInvites"} />, resizeHandles: availableHandles, x: 10, y: 6, w: 3, minW: 2, maxW: 4, h: 3, minH: 3, maxH: 12 },
  ]);
